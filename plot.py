...
def get_experiments_dataframe(config):
    ...
    result_base_dir = 'results/' + config['name'] + '/emulab_experiments/'
...
label_to_color_map = {
    'BBR':        (0.36, 0.54, 0.66), # Air force blue
    'BBRrandom':  (0.0, 1.0, 1.0),    # Aqua blue
    'RENO':       (0.53, 0.66, 0.42), # Asparagus green
    'CUBIC':      (1.0, 0.44, 0.37),  # Bittersweet red
    'RENO/CUBIC': (0.71, 0.65, 0.26), # Brass green
    'RENO/CUBICfriendly': (0.59, 0.29, 0.0), # Brown
    'BBR/RENO':   (0.74, 0.2, 0.64),  # Byzantine purple
    'BBR/CUBIC':  (0.44, 0.16, 0.39), # Byzantium dark purple
    'RENOlazy':   (0.16, 0.44, 0.39), # Dark green
    'BBR2':       (0.0,  0.75, 1.0),  # Capri light blue
    'BBR/BBR2':   (0.0,  0.28, 0.67), # ?
    'BBR2/RENO':  (0.98, 0.81, 0.69), # Apricot light rose
    'BBR2/CUBIC': (0.87, 0.26, 0.51), # Blush dark rose
    'STABLE':     (0.0, 0.0, 0.0)
}
...
def plot_metric(plot_handle, data_source, data_frame, x_param, y_param, z_param, other_params, plot_config):
    ...
    for other_param_name, other_param_val in other_params:
        ...
        print(other_param_name, other_param_val)
    ...
    for z_val in z_lines.keys():
        if data_source == 'Model':
            line_map[z_val] = plot_handle.plot(available_x_for_z[z_val], \
                                               ...
                                               linestyle='-', marker=label_to_marker_map[z_val], \
                                               color=label_to_color_map[z_val], mfc='none', linewidth=0.8)[0]
        else:
            line_map[z_val] = plot_handle.errorbar(available_x_for_z[z_val], \
                                                   ...
                                                   color=label_to_color_map[z_val], mfc='none', linewidth=0.8, capsize=5)[0]
